...
This fork adds the possibility to specify some load_balancing nginx behaviour:
 ...
 - can set access from localhost checking the env 'LOCALHOST' from the container

### Usage
    
Start any container you want proxied with an env var `VIRTUAL_HOST=subdomain.youdomain.com`

    $ docker run -e VIRTUAL_HOST=foo.bar.com -e LOAD_BALANCER_LEAST_CONN=true -e LOAD_BALANCER_IP_HASH=true -e LOCALHOST=true -e MARK_BACKUP_AFTER=2...
